...
import { ref, onValue, off, push, serverTimestamp, set, query, orderByChild, limitToLast } from "firebase/database";
      ...
      >
        {!isCurrentUserMessage && (
          <p className={cn(
            ...
            isChohanGenAiMessage ? "text-accent-foreground/80" : "text-primary"
            )}>
...
export default function LiveChatInterface() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(messagesQuery, (snapshot) => {
        ...
        const loadedMessages: LiveMessage[] = data ? Object.keys(data).map(key => ({...data[key], id: key})) : [];
        const sortedMessages = loadedMessages.sort((a, b) => a.timestamp - b.timestamp);
        
        const finalMessages = isInitialLoad ? [welcomeMessage, ...sortedMessages] : sortedMessages;
      ...
      if (aiPrompt) {
        ...
        try {
          ...
          const aiInput: ManageConversationContextInput = {
            userInput: `In a group chat, user @${currentUser.nickname || currentUser.username} just said to you: "${aiPrompt}"`,
    ...
    <div className="flex flex-col h-screen bg-transparent shadow-2xl rounded-lg overflow-hidden lg:mx-auto lg:max-w-4xl border border-border/30">
      ...
      <div className="p-4 border-t border-border/50 bg-card/70 backdrop-blur-md shadow-lg">
        ...
        >
          <Input
            type="text"
            placeholder="Send a message... (@chohangenai for AI)"
            value={inputValue}
